...
%% ---- Main loop: process each XY quadrant ---------------------------------
    ...
    for ai = 1:numel(angles)
        ...
        % Export formally as 1D CW Complex (CSV format)
        export_cw_complex_2d(graph_data, fullfile(output_dir, prefix));
    
        % Export uniquely structured as mathematical GeoJSON-like features
        export_cw_complex_json(graph_data, fullfile(output_dir, prefix));
    end
    ...
    ReconFull(r1:r2, c1:c2, :) = V_recon_d;
