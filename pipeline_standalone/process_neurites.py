...
def process_tile_gpu(args):
    ...
    with cp.cuda.Stream():
        ...
        # Hybrid threshold: Strict percentile, but MUST be above an absolute minimum vesselness (0.015).
        # This completely eliminates the tiling artifact where empty tiles pick up pure noise!
        perc = cp.percentile(vesselness_smooth, neurite_params.get('threshold_percentile', 99.5))
        thresh = cp.maximum(perc, cp.float32(0.015))
            
        binary_neurite = vesselness_smooth > thresh
        
        # Fast sparse structure (7 elements instead of 125)
        struct_fast_gpu = cp.asarray(ndi.generate_binary_structure(3, 1))
        
        # Aggressive 5-iteration closing to completely fill hollow cores in thick dendrites (prevents parallel lines)
        for _ in range(5):
