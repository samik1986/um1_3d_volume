...
def run_pipeline(input_tiff, output_dir, workers, no_vis=False):
    ...
    if os.path.exists(swc_path):
        print(f"\nSkeleton SWC already exists at: {swc_path}")
        ...
        print("\n--- Step 3: Skeletonization to SWC ---")
        cmd3 = ["python", "-u", "skeletonize_only.py", "--neurite_mask", mask_path, "--soma_mask", soma_labels_path, "--output_swc", swc_path]
        subprocess.run(cmd3, check=True)
        # (Step 4 SWC Conversion skipped because it is now natively handled by Step 3)

    
    # 5. Viewer Launch
    if not no_vis:
        print("\n--- Step 5: Launching Napari Proofreading Viewer ---")
        cmd5 = ["python", "-u", "utils/viewer.py", "--raw", input_tiff, "--cw", cw_path, "--mask", mask_path, "--somas", soma_labels_path]
        subprocess.run(cmd5)
    else:
        print("\n--- Step 5: Skipping Napari Viewer (--no-vis flag passed) ---")
