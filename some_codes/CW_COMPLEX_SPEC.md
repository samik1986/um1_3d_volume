...
*   **`radius`** (Array of Floats, Optional): Radii measured along the 1-cell skeleton.

### 3. The 2-Cells (`cells_2_surfaces`, Optional)
Encodes 2D surface patches wrapping around the 1D skeleton.
*   **`surface_id`** (Integer): Unique tracking abstraction.
*   **`parent_line_id`** (Integer): Reference to the 1-cell linestring this surface wraps.
*   **`geometry`** (Array): Surface mesh definitions (e.g., vertices and faces or boundary voxels).

### 4. The 3-Cells (`cells_3_volumes`, Optional)
Encodes the 3D solid volume enclosing the skeletonized structure.
*   **`volume_id`** (Integer): Unique tracking abstraction.
*   **`parent_line_id`** (Integer): Reference to the 1-cell linestring this volume corresponds to.
*   **`volume_voxels`** (Integer): Total volume measured in voxels.
